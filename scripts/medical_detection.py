#!/usr/bin/env python3
"""
Medical Products Detection Script

This script uses the trained YOLO model to detect medical products
in images from Telegram channels.
"""
...
class MedicalProductsDetector:
    ...
    def _load_model(self):
        ...
        try:
            ...
            # Fix for PyTorch 2.6 compatibility - monkey patch torch.load
            import torch
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Medical Products Detection")
    parser.add_argument('--model', type=str, 
                       default="/Users/mikiyasegaye/MK_Lab/10 Academy/From-Raw-Telegram-Data-to-an-Analytical-API/runs/detect/yolov8n-medical-products3/weights/best.pt",
                       help='Path to trained YOLO model')
    parser.add_argument('--input', type=str, default='data/images',
                       help='Input image or directory')
    parser.add_argument('--output', type=str, default='results/medical_detections',
                       help='Output directory for annotated images and results')
    parser.add_argument('--confidence', type=float, default=0.25,
                       help='Confidence threshold (default: 0.25)')
    parser.add_argument('--save-results', type=str, default='results/detection_results.csv',
    ...
    try:
        ...
        if input_path.is_file():
            ...
            if detections:
                ...
                output_file = output_path / f"annotated_{input_path.name}"
                cv2.imwrite(str(output_file), annotated_image)
                print(f"✅ Annotated image saved: {output_file}")
        
        elif input_path.is_dir():
            # Batch detection
            results_df = detector.detect_batch(str(input_path), str(output_path))
